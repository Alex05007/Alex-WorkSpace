...
if($tit = $titlu2->fetch()){
	if ($_GET['fa'] == "rasp" and $tit['TestCreator'] != $_COOKIE['login']) {
	...
	//if ($row['TestCreator'] != $_COOKIE['login']) {
		if ($tit['deLa'] != 0 and $tit['panaLa'] != 0) {
			if (date("Gi") < $tit['deLa'] or $tit['panaLa'] < date("Gi")) {
				header("Location: raspunde.php?time=true&id=" . $_GET['id']);
				die();
			}
...
$puncte = 0;
if ($_GET['r'] != "") {
	$ra = "SELECT * FROM question WHERE Testid='" . $_GET['id'] . "'";
	$ras = $conn->query($titlu);
	while ($r = $ras->fetch()){
		$punctaj[] = $r['Puncte'];
	}
	$ra = "SELECT * FROM raspunsuri WHERE test='" . $_GET['id'] . "' and id='rasp'";
	$ras = $conn->query($titlu);
	if($r = $ras->fetch()){
		$rasp_corect = explode(";", $r['raspunsuri']);
		$rasp = explode(";", $_GET['r']));
		for ($i=0; $i<count($rasp_corect); $i++) {
			if ($rasp[$i] == $rasp_corect[$i]) {
				$puncte = $puncte + intval($punctaj[$i]);
			}
		}
	}
	...
	if ($tit['deLa'] != 0 and $tit['panaLa'] != 0) {
		if (date("Gi") < $tit['deLa'] or $tit['panaLa'] < date("Gi")) {
			header("Location: http://gnets.myds.me/work/forms/?id=" . $_GET['id']);
			die();
		} else {
			...
			$stmt = $conn->query($sql);
			if ($puncte != 0) {
				header("Location: http://gnets.myds.me/work/forms/raspunde.php?sent=true&id=" . $_GET['id']);
				die();
			} else {
				header("Location: http://gnets.myds.me/work/forms/rezolvat.php?id=" . $_GET['id'] . "&p=" . $puncte);
				die();
			}
		}
	}
		...
		if ($_GET['sId'] == "rasp") {
			$sql = "INSERT INTO raspunsuri (id, nume, raspunsuri, test, parasiri, comentariu) VALUES ('" . $_GET['sId'] . "', '" . "Answers" . "', '" . $_GET['r'] . "', '" . $_GET['id'] . "', '" . $_GET['parasiri'] . "', '')";
			$stmt = $conn->query($sql);
			header("Location: http://gnets.myds.me/work/forms/edit.php?id=" . $_GET['id']);
			die();
		} else {
			$sql = "INSERT INTO raspunsuri (id, nume, raspunsuri, test, parasiri, comentariu) VALUES ('" . $_GET['sId'] . "', '" . $_GET['nume'] . "', '" . $_GET['r'] . "', '" . $_GET['id'] . "', '" . $_GET['parasiri'] . "', '')";
			$stmt = $conn->query($sql);
...
function submitForms(intr_nr){
	...
	if (ok == true) {
		...
		if (getCookie("ano") == "false") {
			...
			if (nume == "") {
				document.getElementById('name-wr').style.border="#F00 1px solid";
			} else {
				$.ajax
				  ({
					type: "POST",
...
<?php 
	if (!isset($_GET['sent']) and !isset($_GET['time'])) {
?>
<body onload="loadStart()" onblur="trisare()">
<?php if(isset($_COOKIE['login'])) { include "../nav.php"; } ?>
<div class="top">
	<?php
	...
	<?php
		...
		if ($ano == "false") {
			if (isset($_COOKIE['un']) and isset($_COOKIE['login'])) {
				echo "<input type='text' id='nume' value='" . $_COOKIE['un'] . "' readonly='readonly'>";
			} else {
...
while($row = $stmt->fetch()){
	...
	if ($row['Tip'] == "completare") {
		...
		echo '<div class="input-data">
			<input type="text" required="required" name="rasp' . $intr_nr . '" value="' . $r[1] . '" onkeydown="changeForm()">
	...
	if ($row['Tip'] == "info") {
		...
		echo "<h3><strong>" . $row['Intrebare'] . "</strong></h3></div><br />"; 
		$intr_nr = $intr_nr - 1;
	}
	$intr_nr++;
}
...
?>
<?php echo '<div style="text-align:left;" id="submit"><button class="mdc-button mdc-button--raised" onclick="submitForms(\'' . $intr_nr . '\')" >
...
<?php if (!isset($_GET['sent']) and isset($_GET['time'])) {?>
		<?php if(isset($_COOKIE['login'])) { include "../nav.php"; } ?>
        <body onload="loadStart()">
        <div class="top">
            <?php
            ...
            ?>
            <h4 style="font-weight:inherit;">The form doesn't accept respenses now!</h4>
            <br />
        </div>
        <br /><br /><br />
        <a class="mdc-button" href="create.php">
